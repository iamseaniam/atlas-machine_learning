...
class ReplayMemory:
    def __init__(self, capacity, device='cpu'):
        self.cappacity = capacity
        self.memory = []
        self.position = 0
        self.device = device
        self.memory_max_report = 0

    def insert(self, transition):  # think of transition as a tuple
        transition = [item.to('cpu')
                      for item in transition]  # send all items to cpu

        if len(self.memory) < self.cappacity:
            self.memory.append(transition)
        else:
            self.memory.remove(self.memory[0])
            self.memory.append(transition)

    def sample(self, batch_size=32):  # sets default batch size to 32 for a good normal num
...
class Agent:
    ...
    def __init__(self, model, device='cpu', epsilon=1.0, min_epsilon=0.1,
                 nb_warmup=10000, nb_actions=None, memory_capacity=10000,
        ...
        self.epsilon = epsilon  # ! seleclting random action. that is the epsilon greedy policy
    ...
    def get_action(self, state):
        if torch.rand(1) < self.epsilon:
            ...
            # [0.11, 0.45, 0.23, 0.21] will choose highest reward.
            # would return actio 1. the 0.45
        ...
        for epoch in range(1, epochs + 1):
            ...
            while not done:
                ...
                if self.memory.can_sample(self.batch_size):
                    state_b, action_b, reward_b, done_b, next_state_b = self.memory.sample(
                        self.batch_size)
